...
async def testbench(ctx):
    test_data = [ 3, 2, 7, 6, 8, 1, 0, 2, 4, 0, 3, 2, 7, 6, 8, 0]
...
if __name__ == "__main__":
    dut = AsyncFIFO(width=8, depth=16) # depth must power of 2 depths

    sim = Simulator(dut)
    # async fifo no need sync domain
    sim.add_clock(Period(MHz=1), domain="write")
    sim.add_clock(Period(MHz=10), domain="read")
    sim.add_testbench(testbench)
    with sim.write_vcd("AsyncFIFO.vcd"):
        sim.run()
