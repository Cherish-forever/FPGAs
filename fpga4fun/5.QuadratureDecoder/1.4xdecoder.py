...
class Decoder4x(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        quad_a_delayed = Signal()
        quad_b_delayed = Signal()

        m.d.sync += [
            quad_a_delayed.eq(self.quad_a),
            quad_b_delayed.eq(self.quad_b),
        ]

        count_enable = Signal()
        m.d.comb += [
            count_enable.eq(self.quad_a ^ quad_a_delayed ^ self.quad_b ^ quad_a_delayed),
            self.direction.eq(self.quad_a ^ quad_b_delayed)
        ]
